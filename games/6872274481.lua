...
run(function()
	...
	task.spawn(function()
		...
		projectileRemote = bedwars.Client:Get(_remotes.FireProjectile).instance
		pcall(function()
	...
	local function FireAttackRemote(weapon, entityInstance, selfPos, targetPos)
		...
		local pos = selfPos + dir * math.max(delta - 14.399, 0)
	...
	local ConeParts = {}
	local CONE_SEGMENTS = 40

	local function destroyRangeCircle()
		for _, p in ConeParts do
			pcall(function() p:Destroy() end)
		end
		table.clear(ConeParts)
	end

	local function createRangeCircle()
		destroyRangeCircle()
		if not (RangeCircle and RangeCircle.Enabled) then return end
		for i = 1, CONE_SEGMENTS do
			...
			p.Size = Vector3.new(0.2, 0.05, 1)
			p.Transparency = 0.65
			p.Parent = gameCamera
			p:SetAttribute('gamecore_GameQueryIgnore', true)
			ConeParts[i] = p
		end
	end

	local function updateRangeCircle()
		if #ConeParts == 0 then return end
		if not entitylib.isAlive then
			for _, p in ConeParts do p.Transparency = 1 end
			return
		end
		local root = entitylib.character.RootPart
		...
		local hasTarget = store.KillauraTarget ~= nil
		local c3 = hasTarget and Color3.fromRGB(255, 40, 40)
			or Color3.fromHSV(BoxSwingColor.Hue, BoxSwingColor.Sat, BoxSwingColor.Value)

		local base = root.Position - Vector3.new(0, hum.HipHeight - 0.06, 0)
		local flat = root.CFrame.LookVector * Vector3.new(1, 0, 1)
		if flat.Magnitude < 0.001 then return end
		local facing = flat.Unit
		local span = math.rad(math.clamp(AngleSlider.Value, 1, 360))
		local len = math.max(AttackRange.Value, 0.1)
		local step = span / CONE_SEGMENTS
		local segW = 2 * len * math.tan(step / 2) * 1.02

		for i, p in ConeParts do
			local ang = -span / 2 + step * (i - 0.5)
			local dir = CFrame.Angles(0, ang, 0) * facing
			p.Size = Vector3.new(segW, 0.05, len)
			p.CFrame = CFrame.lookAt(base + dir * (len / 2), base + dir * len)
			p.Color = c3
			p.Transparency = 0.65
		end
	end
	...
	local function computeAttackData()
		...
		if Mouse and Mouse.Enabled then
			...
			local recentSwing = LegitAura and LegitAura.Enabled and lastSwing and (tick() - lastSwing) <= 0.2
		...
		if LegitAura and LegitAura.Enabled then
			local lastSwing = (bedwars.SwordController and bedwars.SwordController.lastSwing) or 0
	...
	local function getProjectiles()
		...
		for _, item in store.inventory.inventory.items do
			...
			if pmeta and pmeta.arrow and pmeta.launchVelocity then
	...
	local function fhRestoreSword()
		...
		local token = fhRestoreToken
		check.Value = sw.tool
		task.spawn(function()
			pcall(function() bedwars.Client:Get(remotes.EquipItem):CallServerAsync({hand = sw.tool}) end)
			for _ = 1, 3 do
				task.wait(0.06)
				if token ~= fhRestoreToken then return end
				if not sw.tool.Parent then return end
				if not Killaura or not Killaura.Enabled then return end
				if check.Value ~= sw.tool or (store.hand and store.hand.tool ~= sw.tool) then
					check.Value = sw.tool
					pcall(function() bedwars.Client:Get(remotes.EquipItem):CallServerAsync({hand = sw.tool}) end)
				else
					return
	...
	local function smoothedVelocity(ent, targetPart)
		...
		if not _fhVelHistory[key] then
			_fhVelHistory[key] = rawVel
		else
			_fhVelHistory[key] = _fhVelHistory[key]:Lerp(rawVel, 0.35)
		end
		return _fhVelHistory[key], rawVel
	end
	...
	local function shootProjectile(item, ammo, projectile, itemMeta, selfPos, ent, ignoreSwitch)
		...
		if not ignoreSwitch then
			fhBusy = true
			store._fhBusySince = workspace:GetServerTimeNow()
		...
		if multiAt and chargePct >= 1 then
		...
		do
			...
			local speed = tonumber(meta.launchVelocity) or 0
			local grav = tonumber(meta.gravitationalAcceleration) or 196.2
			...
			elseif meta.arrow then
				if grav <= 40 and speed >= 200 and dist < 60 and head then
					aimPart = head
					aimOffset = 0
				else
					aimOffset = hip * 0.15
		...
		sharedFastHitsRayParams.FilterDescendantsInstances = {lplr.Character, gameCamera, ent.Character}
		...
		local calc, _impact, flightTime = prediction.SolveTrajectory(
			originPos, projSpeed, gravity, leadPos, rawVel,
			playerGravity, ent.HipHeight or 2,
			ent.Jumping and 42.6 or nil, sharedFastHitsRayParams,
			nil, targetPart.Position, targetPart, nil, true
		)
		...
		if meta.arrow then
			shootPos = (CFrame.new(originPos, calc) * CFrame.new(Vector3.new(-(bedwars.BowConstantsTable.RelX or 0), -(bedwars.BowConstantsTable.RelY or 0), -(bedwars.BowConstantsTable.RelZ or 0)))).Position
		end
		do
		...
		targetinfo.Targets[ent] = tick() + 1
		ProjectileDelay[item.itemType] = tick() + (tonumber(itemMeta.fireDelaySec) or 0.5) + drawTime

		if not isFireball then
			bedwars.ProjectileController:createLocalProjectile(
				meta, ammo, projectile, shootPos, id, dir * projSpeed,
				{drawDurationSec = drawTime, drawDurationSeconds = drawTime}
			)
		end
	...
	local function shootKitWeapon(item, ammo, projectile, selfPos, ent)
		...
		local extPos = chestPos
		local adjustedVel = smoothVel

		sharedFastHitsRayParams.FilterDescendantsInstances = {lplr.Character, gameCamera, ent.Character}

		local calc = prediction.SolveTrajectory(
			selfPos, projSpeed, gravity, extPos, rawVel,
			playerGravity, ent.HipHeight or 2,
			ent.Jumping and 42.6 or nil, sharedFastHitsRayParams,
			nil, targetPart.Position, targetPart, nil, true
		)
		...
		pcall(function() frostyGunRemote:FireServer({keyHold = true}) end)
		pcall(function() frostyGunRemote:FireServer({keyHold = false}) end)
		...
		task.spawn(function()
			local res = projectileRemote:InvokeServer(
				item.tool, nil, projectile, muzzlePos, firePos,
				dir * projSpeed, id,
				{shotId = httpService:GenerateGUID(false), drawDurationSec = drawDur},
				workspace:GetServerTimeNow() - ping
			)
			if res then
				pcall(function() res.Parent = replicatedStorage end)
	...
	local function getMageSpell()
		local ok, spell = pcall(function()
			...
			if not element then return 'mage_spell_base' end
			local elLower = string.lower(tostring(element))
			local unlocked = lplr:GetAttribute(elLower)
			if not unlocked or unlocked == 0 then return 'mage_spell_base' end
	...
	local kitAmmoMap = {
		frost_staff = {base = 'frosty_snowball', leveled = true},
		ninja_chakram = {base = 'ninja_chakram', leveled = true},
		mage_spellbook = {dynamic = 'mage'},
	}
		...
		for _, item in store.inventory.inventory.items do
			...
			if itype:find('fireball') then
				...
				if meta and meta.projectileSource then
					return {item, itype, meta.projectileSource.projectileType(itype), meta}
				end
			end
		end
		return nil
	...
	local function shootGloop(item, ent)
		if not ent or not ent.RootPart then return false end
		local key = tostring(ent)
		local now = tick()
		...
		if tracked and tracked.target == ent then
			if tracked.gloopedUntil and now < tracked.gloopedUntil then return false end
			if tracked.lastShot and (now - tracked.lastShot) < 3 then return false end
		end

		if isGlooped(ent) then
			gloopTracker[key] = {target = ent, lastShot = now, gloopedUntil = now + 8}
			return false
		end

		local myRoot = entitylib.character and entitylib.character.RootPart
		if not myRoot then return false end
		if (ent.RootPart.Position - myRoot.Position).Magnitude > 45 then return false end
		...
		local aimPos = targetPart.Position - Vector3.new(0, (ent.HipHeight or 2) * 0.6, 0)

		sharedFastHitsRayParams.FilterDescendantsInstances = {lplr.Character, gameCamera, ent.Character}

		local originPos = selfPos + Vector3.new(0, 1.5, 0)
		local calc, _gImpact, gFlight = prediction.SolveTrajectory(
			originPos, gSpeed, gGrav, aimPos, rawVel,
			playerGravity, ent.HipHeight or 2,
			ent.Jumping and 42.6 or nil, sharedFastHitsRayParams,
			nil, targetPart.Position, targetPart, nil, true
		)
		local gLife = tonumber(gmeta and gmeta.predictionLifetimeSec) or 2
		if not calc or (gFlight and gFlight > gLife) then return false end
	...
	local function doFastHitsNEW(ent)
		local _stages = 0
		if not ent or not ent.RootPart or not entitylib.isAlive then return end
		local selfPos = entitylib.character.RootPart.Position

		local anyOn = (Gloops and Gloops.Enabled) or (Kits and Kits.Enabled)
			or (Fireball and Fireball.Enabled) or (Arrows and Arrows.Enabled)
		if not anyOn then return end
		if Gloops and Gloops.Enabled then _stages += 1 end
		if Kits and Kits.Enabled then _stages += 1 end
		if Fireball and Fireball.Enabled then _stages += 1 end
		if Arrows and Arrows.Enabled then _stages += 1 end
		for _ = 1, math.max(_stages * 2, 4) do
			if fhStage == 1 then
				fhStage = 2
				if Gloops and Gloops.Enabled then
					local gloopItem = getGloopItem()
					if gloopItem and shootGloop(gloopItem, ent) then return end
				end
			elseif fhStage == 2 then
				fhStage = 3
				if Kits and Kits.Enabled then
					local kw = getKitWeapon()
					if kw and canShoot(kw) and shootKitWeapon(kw[1], kw[2], kw[3], selfPos, ent) then return end
				end
			elseif fhStage == 3 then
				fhStage = 4
				if Fireball and Fireball.Enabled then
					local fb = getFireballItem()
					if fb and canShoot(fb) then
						ProjectileDelay[fb[1].itemType] = tick() + 1
						if shootProjectile(fb[1], fb[2], fb[3], fb[4], selfPos, ent, false) then return end
					end
				end
			else
				fhStage = 1
				if Arrows and Arrows.Enabled then
					...
					if projectiles and #projectiles > 0 then
						for _i = 1, #projectiles do
							fhUsageIndex = fhUsageIndex % #projectiles + 1
							local p = projectiles[fhUsageIndex]
							if canShoot(p) then
								local item, ammo, projectile, itemMeta = unpack(p)
								if shootProjectile(item, ammo, projectile, itemMeta, selfPos, ent, false) then return end
								break
							end
						end
					end
	...
	local function doFastHits()
		...
		local sinceHit = workspace:GetServerTimeNow() - kaLastSrv
		if sinceHit < 0.02 then return end
		if sinceHit > (kaPeriod * 0.45) then return end
		...
		if not fhInAngle(ent) then return end
		if (ent.RootPart.Position - selfRoot.Position).Magnitude > SwingRange.Value then return end
	...
	local function startAutoShootLoop()
		...
		if not ProjectileDelay then ProjectileDelay = {} end
		fhUsageIndex = 1
		fhStage = 1
	...
	local function stopAutoShootLoop()
		...
		if ProjectileDelay then table.clear(ProjectileDelay) end
	...
	local _graceTarget = nil
	local _graceUntil = 0
	local _fastAcqTime = 0
	local _fastAcqRes = nil
	...
	local function inReach(v)
		...
		return (v.RootPart.Position - root.Position).Magnitude <= (AttackRange.Value + 2)
	end
	...
	local function resolveTarget()
		local v = store.KillauraTarget
		if aliveTarget(v) and withinAngle(v) and inReach(v) then
			_graceTarget = v
			_graceUntil = tick() + 0.4
			return v
		end
		if _graceTarget and tick() < _graceUntil then
			local g = _graceTarget
			if aliveTarget(g) and inReach(g) then
				return g
			end
		end
		local now = tick()
		if (now - _fastAcqTime) < 0.03 and _fastAcqRes then
			if not (aliveTarget(_fastAcqRes) and withinAngle(_fastAcqRes) and inReach(_fastAcqRes)) then return nil end
			return _fastAcqRes
		end
		_fastAcqTime = now
		_fastAcqRes = nil
		local list = entitylib.AllPosition({
			Range = AttackRange.Value + 2,
	...
	local function attemptFire()
		...
		if not sword or not meta then
			...
			if type(hspd) == 'number' and hspd >= 0.1 then kaPeriod = hspd end
			return false
		end
		
		local spd = meta.sword and meta.sword.attackSpeed
		kaPeriod = (type(spd) == 'number' and spd >= 0.1) and (spd * 0.984) or HIT_PERIOD
		...
		if not tchar or not tchar.PrimaryPart then return false end
		local selfPos = entitylib.character.RootPart.Position
		local targetPos = tchar.PrimaryPart.Position
		local dist = (targetPos - selfPos).Magnitude
		if dist > AttackRange.Value + 2 then return false end
		...
		if AirHit and AirHit.Enabled and AirHitsChance and AirHitsChance.Value < 100 then
			...
			if hum then
				...
				if inAir then
					if math.random(1, 100) > AirHitsChance.Value then return false end
				end
			end
		end

		if sword.tool.Name:find('summoner_claw') then
			pcall(function() KaidaController:request(tchar) end)
			return false
		end

		if LegitAura and LegitAura.Enabled then
			local ls = (bedwars.SwordController and bedwars.SwordController.lastSwing) or 0
			if ls == lastFiredSwing then return false end
			lastFiredSwing = ls
		end
		...
		kaLastSend = srvNow
		if kaLastSrv > 0 then
			kaLastSrv = kaLastSrv + kaPeriod
			if kaLastSrv < srvNow then
				kaLastSrv = srvNow
			end
		else
			kaLastSrv = srvNow
	...
	local function startKillauraFireLoop()
		...
		killauraFireLoop = task.spawn(function()
			...
			while Killaura and Killaura.Enabled do
				...
				local nowc = os.clock()
				local dt = nowc - lastFrame
				lastFrame = nowc
				if dt > 0 and dt < 0.25 then
					frameDelta = frameDelta + (dt - frameDelta) * 0.2
				end
				if LegitAura and LegitAura.Enabled then
					pcall(attemptFire)
				else
					local target = kaPeriod - math.min(frameDelta * 0.5, kaPeriod * 0.06)
					if (workspace:GetServerTimeNow() - kaLastSrv) >= target then
						pcall(attemptFire)
					end
				end
			end
	...
	Killaura = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				if Animation and Animation.Enabled and not (LegitAura and LegitAura.Enabled) and not (identifyexecutor and table.find({'Argon', 'Delta'}, ({identifyexecutor()})[1])) then
					task.spawn(function()
						local started = false
						repeat
							if Attacking and not (LegitAura and LegitAura.Enabled) and (not (Mouse and Mouse.Enabled) or inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1)) then
								if not armC0 then
				...
				local function gatherTargets()
					if _gatherCache and (tick() - _gatherCacheTime) < 0.05 then
						return _gatherCache[1], _gatherCache[2]
					end

					local walls = Targets.Walls.Enabled or nil
					local priority = TargetPriority and TargetPriority.Value or 'Players First'
					...
					local function buildList(range)
						if priority == 'Players First' then
							local r = entitylib.AllPosition({Range = range, Wallcheck = walls, Part = 'RootPart', Players = players, NPCs = false, Limit = 1, Sort = sort})
							if #r > 0 or not npcs then return r end
							return entitylib.AllPosition({Range = range, Wallcheck = walls, Part = 'RootPart', Players = false, NPCs = true, Limit = 1, Sort = sort})
						elseif priority == 'NPCs First' then
							local r = entitylib.AllPosition({Range = range, Wallcheck = walls, Part = 'RootPart', Players = false, NPCs = npcs, Limit = 1, Sort = sort})
							if #r > 0 or not players then return r end
							return entitylib.AllPosition({Range = range, Wallcheck = walls, Part = 'RootPart', Players = true, NPCs = false, Limit = 1, Sort = sort})
						end
						return entitylib.AllPosition({Range = range, Wallcheck = walls, Part = 'RootPart', Players = players, NPCs = npcs, Limit = 1, Sort = sort})
					end

					local attackPlrs = buildList(AttackRange.Value)
					local swingPlrs = AttackRange.Value == SwingRange.Value and attackPlrs or buildList(SwingRange.Value)
					_gatherCache = {swingPlrs, attackPlrs}
				...
				repeat
					if AttackCheck and AttackCheck.Enabled then
						...
						if not triggered and kitChecks then
							for _, check in pairs(kitChecks) do
								local ok, res = pcall(check)
								if ok and res then triggered = true break end
							end
						end
						if triggered then
							Attacking = false
					...
					if RangeCircle and RangeCircle.Enabled and (tick() - (store._kaConeTime or 0)) > 0.033 then
						store._kaConeTime = tick()
						pcall(updateRangeCircle)
					end

					table.clear(attacked)
					local sword, meta = getAttackData()
					if not sword and not shouldContinueSwinging() then
						if Attacking then
							Attacking = false
							getgenv().Attacking = false
							store.KillauraTarget = nil
					...
					if sword then
						...
						if swordName ~= _cachedSwordType and not fhBusy then
						...
						local function passesAngle(v)
							if AngleSlider.Value >= 360 then return true end
							local flat = (v.RootPart.Position - selfpos) * Vector3.new(1, 0, 1)
							if flat.Magnitude <= 1.0 then return true end
							return math.acos(math.clamp(localfacing:Dot(flat.Unit), -1, 1)) <= maxAngle
						end
						...
						if not hasValidSwingTargets and not hasValidAttackTargets and Attacking then
							Attacking = false
							getgenv().Attacking = false
							store.KillauraTarget = nil
						end
						...
						if shouldSwing or isLegitAura then
							if not fhBusy and store.hand and store.hand.tool ~= sword.tool and (tick() - (store._kaSwitch or 0)) > 0.25 then
							...
							if chosen then
								...
								local delta = chosen.RootPart.Position - selfpos
								...
								if vapeTargetInfo and vapeTargetInfo.Targets then
									vapeTargetInfo.Targets.Killaura = {
										Humanoid = {Health = chosen.Health, MaxHealth = chosen.MaxHealth},
										Player = chosen.Player
									}
								end
							end

							if not isClaw and AnimDelay <= tick() and not (LegitAura and LegitAura.Enabled) then
								local allowSwingAnim = not (Swing and Swing.Enabled)
									and (not (Mouse and Mouse.Enabled) or inputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1))
					...
					if Face and Face.Enabled and attacked[1] and entitylib.isAlive then
						pcall(function()
							local root = entitylib.character.RootPart
							local vec = attacked[1].Entity.RootPart.Position * Vector3.new(1, 0, 1)
							local targetCFrame = CFrame.lookAt(root.Position, Vector3.new(vec.X, root.Position.Y + 0.001, vec.Z))
							local speed = FaceSpeed and FaceSpeed.Value or 15
							if root.CFrame.LookVector:Dot(targetCFrame.LookVector) < 0.9995 then
								root.CFrame = root.CFrame:Lerp(targetCFrame, math.clamp(speed / 100, 0.01, 1))
							end
						end)
					end
	...
	Sort = Killaura:CreateDropdown({Name = 'Target Mode', List = getSortList({'Damage', 'Distance'})})
	...
	ContinueSwinging = Killaura:CreateToggle({
		...
		Tooltip = 'keeps swingin ur sword after the guy runs off. looks way less sus fr',
		Function = function(callback)
	...
	SwingTime = Killaura:CreateToggle({
		...
		Tooltip = 'lets u pick how fast the sword swings urself'
	})
	...
	Animation = Killaura:CreateToggle({
		...
		Function = function(callback)
			if AnimationMode then AnimationMode.Object.Visible = callback end
			if AnimationTween then AnimationTween.Object.Visible = callback end
			if AnimationSpeed then AnimationSpeed.Object.Visible = callback end
	...
	Limit = Killaura:CreateToggle({
		...
		Tooltip = 'only hits when u actually got the sword out'
	})
		...
		while _watchersActive and vape.Loaded do
			...
			if bedwars.AbilityController then
				...
				if ok then
					...
					if wasAvailable and not nowAvailable and availSince > 0 and (tick() - availSince) > 1 then
		...
		while _watchersActive and vape.Loaded do
			...
			if bedwars.AbilityController then
				...
				if ok1 then
					...
					if nowStomp and not wasStomp then stompSince = tick() end
					if wasStomp and not nowStomp and stompSince > 0 and (tick() - stompSince) > 1 then
				...
				if ok2 then
					...
					if nowKick and not wasKick then kickSince = tick() end
					if wasKick and not nowKick and kickSince > 0 and (tick() - kickSince) > 1 then
	...
	AttackCheck = Killaura:CreateToggle({
		Name = 'Attack Check',
		Tooltip = 'turns ka off in stuff that gets u banned like sophia n sleep',
		Default = false
	})
	...
	FastHits = Killaura:CreateToggle({
		Name = 'Fast Hits',
		Default = false,
		Tooltip = 'chucks arrows n kit stuff at ur target while u swingin on em',
		Function = function(call)
			if ArrowCharge then ArrowCharge.Object.Visible = call end
			if LegitSwitch then LegitSwitch.Object.Visible = call end
			if Kits then Kits.Object.Visible = call end
			if Arrows then Arrows.Object.Visible = call end
			if Gloops then Gloops.Object.Visible = call end
			if Fireball then Fireball.Object.Visible = call end
			if call then
	...
	LegitSwitch = Killaura:CreateToggle({
		...
		Darker = true,
		Visible = false,
		Tooltip = 'swaps items like a real player would, less sus'
	})
	Kits = Killaura:CreateToggle({
		...
		Darker = true,
		Visible = false,
		Tooltip = 'uses ur kit weapon too like frost staff or chakram'
	})
	Arrows = Killaura:CreateToggle({
		...
		Darker = true,
		Visible = false,
		Tooltip = 'turn this off n it wont shoot no arrows at all'
	})
	Gloops = Killaura:CreateToggle({
		...
		Darker = true,
		Visible = false,
		Tooltip = 'auto gloops em if u got some. remembers who u already got so it dont spam the same guy'
	})
	Fireball = Killaura:CreateToggle({
		...
		Darker = true,
		Visible = false,
		Tooltip = 'chucks fireballs at em too if u got some. hits hard but watch out u can blow urself up gng'
	})
	...
	task.defer(function()
		...
		if FastHits and FastHits.Enabled then
			if ArrowCharge then ArrowCharge.Object.Visible = true end
			if LegitSwitch then LegitSwitch.Object.Visible = true end
			if Kits then Kits.Object.Visible = true end
			if Arrows then Arrows.Object.Visible = true end
			if Gloops then Gloops.Object.Visible = true end
			if Fireball then Fireball.Object.Visible = true end
		end
	end)
end)
